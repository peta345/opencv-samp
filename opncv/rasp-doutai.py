...
if __name__ == '__main__':
    ...
    #slack
    #slack = slackweb.Slack(url = 'https://hooks.slack.com/services/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J')
    ...
    while True:
        ...
        # 背景差分の取得。ここで関数がコール
        im_bg,im_in,im_mask,im_edge = bg_diff("bg.jpg",frame,25,blur=7)
        # 各画像の表示(windowを生成する)
        # print type(im_mask)
        cv2.imwrite('bg.jpg',frame)
        print np.ndarray.sum(im_mask)
        pix = np.ndarray.sum(im_mask)
        if pix < 200000000:
            Play()
            time.sleep(1.5)
            pix =  250000000
